...
- (void)setupManager
{
    ...
    ESTBeaconRegion* region = [[ESTBeaconRegion alloc] initWithProximityUUID: MACTS_PROXIMITY_UUID
...
- (IBAction)getBeaconPollSpeed:(id)sender {
    ...
    [self.beacon1 disconnectBeacon];
    [self.beacon1 connectToBeacon];
   [self.beacon1 readBeaconAdvIntervalWithCompletion:^(unsigned short value, NSError *error){NSLog([NSString stringWithFormat:@"Poll 2 Interval: %d", value]);}];
    [self.beacon1 disconnectBeacon];
}
    ...
    if([beacons count] > 0)
    {
     ...
     //   {
       ...
       // }
        else
        {
            for (ESTBeacon* cBeacon in beacons)
            {
                ...
                // update beacon it same as selected initially
                //WWC change to beacon1
                if([self.beacon1.major unsignedShortValue] == [cBeacon.major unsignedShortValue] &&
                [self.beacon1.minor unsignedShortValue] == [cBeacon.minor unsignedShortValue])
                {
 
                    // self.beacon2 = cBeacon;
                    //WWC ChNAGE THE BEACON
                    NSLog([NSString stringWithFormat:@"Amplitude Major:Minor %@ : %@  -- %@ -- %@", [self.beacon1.major stringValue], [self.beacon1.minor stringValue], self.beacon1.macAddress, [self.beacon1.distance stringValue]] );
                }
            }
        }
      ...
      //  float distFactor2 = 1 - ([self.beacon2.distance floatValue] / 20.);
        ...
        self.distance2Label.text = [NSString stringWithFormat:@"%@ :: %f",[self.beacon1.distance stringValue], distFactor2 ];
...
//WWC obsolete with iTheremin.pd
